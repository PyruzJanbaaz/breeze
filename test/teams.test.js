...
contract('Teams', (accounts) => {
  ...
  it('add new team', async () => {
    ...
    assert.equal(
      ++teamsCount,
      parseInt(await contractInstance.getTeamsCount(1)),
    )
  })
  ...
  it('assigen a user to a team', async () => {
    let teamMembersCount = parseInt(
      await contractInstance.getTeamMembersCount(1),
    )
    await contractInstance.assignUserToTeam(1, accounts[0])
    assert.equal(
      ++teamMembersCount,
      parseInt(await contractInstance.getTeamMembersCount(1)),
    )
  })
